...
import { useInsertFinance, useGetFinances, useGetProjects } from "@/utils/api";
...
export default function AddFinancePage() {
    ...
    const { mutate: insertFinance, loading, error } = useInsertFinance();
    ...
    useEffect(() => {
        if (financesData?.data) {
            setExistingNames(
                financesData.data
                    .map(finance => finance.fields?.Name?.value)
                    .filter(Boolean)
            );
        }
    }, [financesData]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await insertFinance({ fields, type });
            
            if (result?.data) {
                setMessage(`${type === 'income' ? 'Income' : 'Expense'} added successfully!`);
                setShowToast(true);
            ...
            setMessage(err.message || 'Failed to add finance entry');
            setShowToast(true);
        }
    };
            ...
            <form onSubmit={handleSubmit}>
                <button 
                    ...
                    style={{
                        padding: '10px 24px',
                        fontSize: '1rem',
                        background: isFormValid && !loading ? 'var(--button-bg)' : 'var(--unselected)',
                        boxShadow: '0 2px 12px rgba(0,0,0,0.08)',
                        borderRadius: '8px',
                        color: isFormValid && !loading ? 'var(--primary)' : 'white',
                        border: isFormValid && !loading ? '2px solid var(--border)' : 'none',
