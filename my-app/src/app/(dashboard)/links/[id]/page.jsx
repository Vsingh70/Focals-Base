...
import { useGetLinks, useGetLink, useUpdateLink, useGetProjects } from "@/utils/api";
...
export default function LinkPage() {
    ...
    const { data: linkData } = useGetLink(id);
    const { data: linksData } = useGetLinks();
    const { data: projectsData } = useGetProjects();
    const { mutate: updateLink, loading, err, data } = useUpdateLink();
    ...
    // Populate form with fetched link data
    useEffect(() => {
        if (linkData?.data) {
            const link = linkData.data;
            setFields({
                Link: { ...fields.Link, value: link.fields?.Link?.value || "" },
                Name: { ...fields.Name, value: link.fields?.Name?.value || "" },
                Category: { ...fields.Category, value: link.fields?.Category?.value || "" },
                Projects: { ...fields.Projects, value: link.fields?.Projects?.value || [] }
            });
        }
    ...
    const handleSubmit = (e) => {
        ...
        // Optionally check for unique name, but allow the current link's name
        if (
            existingNames
                .filter(n => n && n !== linkData?.data?.fields?.Name?.value)
                .map(n => n.toLowerCase())
        ...
        // Update the link
        updateLink({id: linkData.data.id, fields});
        if (!loading && !err) {
