...
export async function PATCH(request) {
  ...
  if (!data || data.length === 0) {
    return Response.json({ data: null, error: 'Gear not found or not updated.' }, { status: 404 });
  }

  if (data.length > 1) {
    return Response.json({ data: null, error: 'Multiple gear objects updated, which should not happen.' }, { status: 500 });
  }
