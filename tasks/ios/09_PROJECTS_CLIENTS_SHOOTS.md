# Task 09 — Projects, Clients, Shoots

## Goal

Build three CRUD modules sharing a common list/detail/form pattern. After this task, every project/client/shoot is fully editable from iOS, clients link to/from system Contacts (with permission), and shoot detail shows the location on a MapKit snapshot.

These three modules are the heaviest hitter for daily use. They share enough patterns that doing them together prevents three rounds of "what should the form layout look like?" debate.
...
- [my-app/src/app/(dashboard)/shoots/](../../my-app/src/app/(dashboard)/shoots/)
...
- Sections:
  - **Basics**: client (link → ClientDetail), category, location, shoot date
  - **Payment**: package price, amount paid, payment status, balance due (computed)
  - **Linked records**: shoots (tap → ShootDetail), contracts (Task 11), finances (Task 10)
...
## Step 4 — Shoots module

### `ShootsScreen` (list)

- Grouped by **Upcoming** / **Past** (sectioned `List`)
- Each row: time, title, client name, location pill
- Tap → `.shootDetail(id)`

### `ShootDetailScreen`

- Header: title, status pill, scheduled time
- **MapKit snapshot** if `location` is set:
  ```swift
  Map {
      Marker(shoot.location ?? "", coordinate: geocoded)
  }
  .frame(height: 180)
  .cardStyle()
  ...
  ```
  Geocode `location` string with `CLGeocoder().geocodeAddressString(_:)`. Cache the result in `CachedShoot` so subsequent renders don't re-hit the geocoder rate limit.
- Sections:
  - **Linked project** (tap → ProjectDetail)
  - **Linked client** (tap → ClientDetail)
  - **Notes**
- Action row: **Add to iOS Calendar** (uses `EventKitMirror.shared.mirror(...)` from Task 08)
- Toolbar: edit, delete

### `ShootForm`

Same `ProjectForm` pattern. Fields:
- Title
- Project (RelatedRecordField, optional)
- Client (RelatedRecordField, optional — auto-populates from project if set)
- Status
- Scheduled at (DatePicker with date+time)
...
Web's behavior (from server actions): deleting a client **nulls out** `client_id` on linked projects/shoots — does NOT cascade-delete. Match this on iOS by:
1. Sending `DELETE /clients/:id` to Supabase as normal — Postgres FK rules handle the nulling.
2. After successful delete, refresh `ProjectsCacheRepository` + `ShootsCacheRepository` to pick up the FK changes.
...
- [ ] Tap row → detail; detail shows linked shoots, contracts, finances
- [ ] Create / edit form round-trips to DB with all fields
...
- [ ] Tap row → detail with linked projects, shoots, inquiries
...
- [ ] Delete client nulls FK on linked projects/shoots (verified by re-querying after delete)

### Shoots
- [ ] List grouped by Upcoming / Past
- [ ] Tap row → detail with map snapshot if location set
- [ ] Map is geocoded once and cached in `CachedShoot`
- [ ] "Add to iOS Calendar" calls `EventKitMirror.mirror` (Task 08)
- [ ] Tapping the map opens the system Maps app to that location
- [ ] Form saves with date+time and duration; cache reflects update

### Shared
- [ ] Form components (`MoneyField`, `DateField`, `StatusField`, `RelatedRecordField`) render consistently across all three modules
...
- 04 (Shell, AppRouter)
- 05 (Cache repos)
- 08 (EventKitMirror — for Shoot detail "Add to iOS Calendar" action)
