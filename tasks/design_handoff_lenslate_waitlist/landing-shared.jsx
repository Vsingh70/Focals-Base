...
function BrowserFrame({ children, height = 560, url = 'app.lenslate.com/projects', glow = true }) {
  return (
...
// ── Browser chrome (responsive, fluid width) — end ───────────
...
function Waitlist({ note, source = 'landing' }) {
  ...
  const submit = async (e) => {
    ...
    const payload = { email: email.trim().toLowerCase(), source, ts: new Date().toISOString() };
...
Object.assign(window, {
  ...
  PhoneFrame, RotatingWord, BrowserFrame,
});
